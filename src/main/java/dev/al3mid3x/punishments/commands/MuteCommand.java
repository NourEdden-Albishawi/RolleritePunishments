...
public class MuteCommand extends BaseCommand {
    ...
    public MuteCommand(Punishments plugin, ConfigManager configManager, Database database) {
    ...
    public boolean execute(CommandSender sender, Command command, String label, String[] args) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            Punishment punishment = new Punishment(target.getUniqueId(), PunishmentType.MUTE, finalReason, -1, staffUUID, System.currentTimeMillis());
            database.savePunishment(punishment);

            Bukkit.getScheduler().runTask(plugin, () -> {
                if (target.isOnline()) {
                    ((Player) target).sendMessage(ChatColor.translateAlternateColorCodes('&', configManager.getMessage("messages.mute").replace("{reason}", finalReason)));
                }

                if (!silent) {
                    Bukkit.broadcastMessage(ChatColor.RED + target.getName() + " has been muted by " + sender.getName() + " for: " + finalReason);
                }
            });
        });
