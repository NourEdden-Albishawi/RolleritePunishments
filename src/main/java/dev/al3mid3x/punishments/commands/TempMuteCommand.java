...
public class TempMuteCommand extends BaseCommand {
    ...
    public TempMuteCommand(Punishments plugin, ConfigManager configManager, Database database) {
    ...
    public boolean execute(CommandSender sender, Command command, String label, String[] args) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            Punishment punishment = new Punishment(target.getUniqueId(), PunishmentType.TEMP_MUTE, finalReason, finalDuration, staffUUID, System.currentTimeMillis());
            ...
            Bukkit.getScheduler().runTask(plugin, () -> {
                if (target.isOnline()) {
                    ...
                    ((Player) target).sendMessage(ChatColor.translateAlternateColorCodes('&', configManager.getMessage("messages.temp_mute").replace("{reason}", finalReason).replace("{time}", timeString)));
                }

                if (!silent) {
                    Bukkit.broadcastMessage(ChatColor.RED + target.getName() + " has been temporarily muted by " + sender.getName() + " for: " + finalReason);
                }
            });
        });
